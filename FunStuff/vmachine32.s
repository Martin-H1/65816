...
PUBLIC	vm_rot
	...
	LDA	TOS_MSW,X	; b
	STA	5,X		; bottom slot = b
	LDA	NOS_LSW,X	; c (TOS)
	STA	TOS_LSW,X	; middle slot = c
	LDA	NOS_MSW,X	; c (TOS)
	STA	TOS_MSW,X	; middle slot = c
	PLA
	STA	NOS_MSE,X	; TOS = a
	PLA
	STA	NOS_LLSSE,X	; TOS = a
	RTS
ENDPUBLIC
...
PUBLIC	vm_rot
	...
	STA	5,X		; bottom slot = b
	LDA	TOS_LSW,X	; c (TOS)
	STA	NOS_LSW,X	; middle slot = c
	LDA	TOS_MSW,X	; c (TOS)
	STA	NOS_MSW,X	; middle slot = c
	PLA
	STA	TOS_MSW,X	; TOS = a
	PLA
	STA	TOS_LSW,X	; TOS = a
...
;------------------------------------------------------------------------------
PUBLIC  vm_twotor
	LDA	NOS_MSW,X		; x1 (NOS)
	PHA
	LDA	NOS_LSW,X		; x1 (NOS)
	PHA
	LDA	TOS_MSW,X		; x2
	PHA
	LDA	TOS_LSW,X		; x2
